...
import { ShieldCheck, MapPin, Car, Clock, CheckCircle2, AlertCircle, Mail, Phone, User as UserIcon, Send } from "lucide-react";
...
const submitInquiryFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Notify owner
    try { await fetch('http://localhost:3000/api/notify-owner', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ type: 'inspection', itemName: data.carYear + ' ' + data.carMake + ' ' + data.carModel, customerName: data.name, customerEmail: data.email, details: 'Phone: ' + data.phone + ', Location: ' + data.location + ', Notes: ' + data.notes }) }); } catch(e) { console.error(e); }
    return { success: true };
  });
      ...
      <div className="bg-dark-steel border-b border-gold/20 py-20">
        <div className="container mx-auto px-4 text-center">
          ...
          <h1 className="text-6xl font-black mb-6 uppercase tracking-tighter italic leading-[0.9]">
            Request <span className="text-gold">Inspection</span>
          </h1>
          <p className="text-titanium text-xl max-w-2xl mx-auto leading-relaxed">
            Tell us about your vehicle and location. We'll review and provide a custom quote based on actual travel costs. No upfront payment.
          </p>
        </div>
      </div>
          ...
          {showSuccess ? (
            <div className="bg-dark-steel border border-green-500/30 rounded-3xl p-12 text-center shadow-2xl">
              ...
              <p className="text-titanium mb-8 max-w-md mx-auto">
                Thank you, {form.name}! Your inspection request for your <span className="text-white font-bold">{form.carYear} {form.carMake} {form.carModel}</span> has been received. A specialist will contact you at <span className="text-white font-bold">{form.email}</span> within 24 hours with a custom quote.
              </p>
            ...
            <div className="bg-dark-steel border border-white/10 rounded-3xl p-8 shadow-2xl">
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="bg-charcoal/50 border border-gold/20 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-center gap-6">
                  <div>
                    <div className="text-[10px] font-black uppercase tracking-widest text-titanium">No Upfront Payment</div>
                    <div className="text-lg font-bold text-white">We'll quote based on your location</div>
                  </div>
                  <button type="submit" disabled={isSubmitting} className="w-full md:w-auto bg-gold text-charcoal px-10 py-4 rounded-xl font-black uppercase text-xs tracking-widest hover:scale-105 transition-all shadow-[0_10px_40px_rgba(201,168,76,0.2)] flex items-center justify-center gap-2 disabled:opacity-50">
          ...
          <div className="mt-16 grid md:grid-cols-2 gap-8">
            <div className="bg-gradient-to-br from-gold/20 to-charcoal border border-gold/30 rounded-3xl p-8">
              ...
              <div className="space-y-4">
                {["Submit your vehicle and contact details", "We review and provide a custom quote", "If approved, we schedule the inspection", "150-point inspection + video walkaround", "PDF report with market valuation"].map((feature, i) => (
            ...
            <div className="bg-dark-steel border border-white/10 rounded-3xl p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="text-white font-black uppercase tracking-widest text-xs">No Obligation</h4>
              </div>
              <p className="text-titanium text-sm leading-relaxed">
                Submitting an inquiry is free and places you under no obligation. We'll provide a transparent quote based on actual travel distance and vehicle complexity. You approve before any payment is made.
              </p>
            </div>
          </div>
        </div>
